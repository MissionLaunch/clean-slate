(function(){
    ...
    function ClientController($rootScope, $scope, $firebaseAuth, userService, $state, $firebaseArray){
        ...
        function saveClient(newClient){
            ...
            //Output information related to client
            console.log("about to save client ");
            console.log(newClient);
            
            //Ensure items in record exists, before save.
            if($scope.records)
                newClient.records = $scope.records;
            else //Save as empty array if undefined
                newClient.records = [];
                
            //Add new record for a client
            var newclientRef = clientRef.push();            
            var dob = new Date(newClient.dobYear, newClient.dobMonth, newClient.dobDay);
            //Set record information for the newly saved client
            newclientRef.set({
            ...
            angular.forEach($scope.records, function(item)
            {
                ...
                */
                ...
                if(item.eligibility === '' && parseInt(eligibilityDate.year) > 0)
                {
                    ...
                    //TESTING - Output whether final results 
